package project.controls;

/**
 * Author Jacky
 */

import java.awt.Color;
import java.awt.Graphics2D;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.awt.event.KeyEvent;
import java.awt.event.KeyListener;
import java.awt.event.MouseEvent;
import java.awt.event.MouseListener;
import java.util.ArrayList;
import javax.swing.Timer;
import java.util.TimerTask;
import java.awt.Color;
import java.awt.Graphics;
import java.awt.Graphics2D;
import java.awt.event.KeyEvent;
import java.awt.event.KeyListener;
import java.awt.event.MouseEvent;
import java.awt.event.MouseListener;
import java.awt.image.BufferedImage;
import java.io.IOException;
import java.net.URL;
import java.util.ArrayList;
import java.util.Random;
import java.util.TimerTask;

import javax.imageio.ImageIO;
import javax.swing.ImageIcon;
import javax.swing.SwingUtilities;

import project.directors.Game;
import project.directors.Screen;


import project.directors.Game;

public class DemoControlsNew extends project.directors.Screen implements KeyListener,MenuInteraction,BattlesInteraction,MouseListener{

	int x = 300;
	int y = 300;
	double walking = 15;
	double speed = walking;
	Boolean selection = false;
	Boolean game = false;
	Boolean menu = false;
	String  s = "Press 1 to select Menu. Press 2 to select Battles.";
	boolean cheatLevel = false; 
	boolean cheatDamage = false;
	boolean cheatSpeed = false;
	boolean cheatAmmo = false;
	boolean battle = false;
	double running = 2; 
	ArrayList<Integer> moving;
	String cheatInput = "";


	public DemoControlsNew(Game game) {
		super(game);
		moving = new ArrayList<Integer>();
	}



	@Override
	public void moveUp() {
		// TODO Auto-generated method stub
	}
	@Override
	public void moveDown() {
		// TODO Auto-generated method stub
	}
	@Override
	public void moveRight() {
		// TODO Auto-generated method stub
	}
	@Override
	public void moveLeft() {
		// TODO Auto-generated method stub	
	}


	@Override
	public void keyPressed(KeyEvent e) {
		int keyCode = e.getKeyCode();
		if(keyCode == KeyEvent.VK_ESCAPE){
			selection = false; 
			game = false;
			menu = false; 
			s = "Press 1 to select Menu. Press 2 to select Battles";
		}
//		if(keyCode == KeyEvent.VK_1){
//			selection = true; 
//			menu = true;
//			s="You are now in menu";
//			System.out.println("nihao");
//		}
		if(keyCode == KeyEvent.VK_1){
			if(!selection){
				selection=true;
				menu=true;
				s="You selected menus controls";
				System.out.println(s);
				return;
			}
		}
		if(keyCode == KeyEvent.VK_2){
			selection = true; 
			game = true;
			s="You are now in Battles";
		}
		if(menu){
			if(keyCode == 13){
				s = "You have pressed Enter";
			};
			if(keyCode == 16){
				s = "You have pressed Shift";
			};
			if(keyCode == 17){
				s = "You have pressed Ctrl";
			};
			if(keyCode == 18){
				s = "You have pressed Alt";
			};
			if(keyCode == 27){
				s = "You have pressed Escape";
			};
			if(keyCode == KeyEvent.VK_Z){
				s = "You confirmed a selection";
			};
			if(keyCode == KeyEvent.VK_X){
				s = "You exited a screen";
			};
		}
		if(game){
			if(keyCode == KeyEvent.VK_3){
				s = "Swarm equipped his SMG";
			}
			if(keyCode == KeyEvent.VK_4){
				s = "Swarm equipped his rifle";
			}
			if(keyCode == KeyEvent.VK_5){
				s = "Swarm equipped his heavy";
			}
			if(keyCode == KeyEvent.VK_6){
				s = "Swarm equipped his rocket launcher";
			}
			if(keyCode == KeyEvent.VK_G){
				s = "Swarm equipped his explosive";
			}
			if(keyCode == KeyEvent.VK_SHIFT){
				if(!moving.contains(moving.indexOf(keyCode))){
					s = "Swarm started to sprint";
					moving.add(keyCode);
					speed*=running;
				}
			}
			if(keyCode == KeyEvent.VK_E){
				s = "Swarm interacted with something on the map";
			}
			if(keyCode == KeyEvent.VK_R){
				s = "Swarm reloaded his weapon";
			}
			if(keyCode == KeyEvent.VK_F){
				s = "Swarm went on stealth mode";
			}
			if(keyCode == KeyEvent.VK_M){
				s = "The player opens a transparent outline of the map";
			}
			if(keyCode == KeyEvent.VK_ESCAPE){
				s = "The player opens the exit menu";
			}	
		}
	}

	public void mousePressed(MouseEvent e){
		if(game){
			if(e.getButton()==MouseEvent.BUTTON1){
				s = "Swarm fired his gun";
			}
			if(e.getButton()==MouseEvent.BUTTON3){
				s = "Swarm fired his alternative shot";
			}
		}
		if(menu){
			if(e.getButton()==MouseEvent.BUTTON1){
				s = "You left clicked confirm";
			}
			if(e.getButton()==MouseEvent.BUTTON3){
				s = "You right clicked to exit a menu";
			}
		}
	}


	ActionListener taskPerformer = new ActionListener(){
		public void actionPerformed(ActionEvent evt){
			s="";
			cheatInput="";
		}
	};
	Timer temp = new Timer(2000,taskPerformer);

	@Override
	public void keyReleased(KeyEvent e) {
		int keyCode = e.getKeyCode();
		cheatInput += KeyEvent.getKeyText((Integer)(keyCode));
		s = cheatInput;
		temp.setRepeats(false);
		temp.start();
		if(game){
		if(cheatInput.contains("C")){
			if(cheatInput.contains("CHEATLEVEL")){
				cheatLevel = true;
				cheatInput = "";
				s ="You have cheated the LEVEL";
			}
			if(cheatInput.contains("CHEATDAMAGE")){
				cheatLevel = true;
				cheatInput = "";
				s ="You have cheated the DAMGE";
			}
			if(cheatInput.contains("CHEATSPEED")){
				cheatLevel = true;
				cheatInput = "";
				s ="You have cheated the SPEED";
			}
			if(cheatInput.contains("CHEATAMMO")){
				cheatLevel = true;
				cheatInput = "";
				s ="You have cheated the AMMO";
			}
		}else{
			cheatInput = null;
		}
		}
	}



	@Override
	public void keyTyped(KeyEvent arg0) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public KeyListener getKeyListener() {
		// TODO Auto-generated method stub
		return null;
	}



	@Override
	public void paintScreen(Graphics2D g2) {
		g2.setColor(Color.blue);
		g2.drawString(s,x, y);
		//System.out.println(s);
	}
	
	
	@Override
	public void mouseClicked(MouseEvent arg0) {
		// TODO Auto-generated method stub
		
	}
	@Override
	public void mouseEntered(MouseEvent arg0) {
		// TODO Auto-generated method stub
	}
	@Override
	public void mouseExited(MouseEvent arg0) {
		// TODO Auto-generated method stub
	}
	@Override
	public void mouseReleased(MouseEvent arg0) {
		// TODO Auto-generated method stub
	}
	
}
